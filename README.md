...
```
원본을 합의 폭으로 고정 → 팝업 끄기 → 좌표·폰트·색 추출
...
| 뷰포트 | 말 없으면 모바일만 `390 × 844`. 데스크톱도면 `1440 × 900`을 따로 |
...
| 하고 싶은 일 | 이렇게 말하기 |
|---|---|
| 페이지 한 장 (모바일) | `이 스킬로 로그인 페이지 원본이랑 맞춰` |
...
여러 장을 돌릴 때는 메인이 큐를 만들고, 워커는 **페이지 하나 × 폭 하나**만 잰다.  
...
flowchart TD
  ...
  F --> G
  G --> H[모바일은 기본 규칙 / 데스크톱은 min-width 안에만]
  H --> I[로컬을 같은 셀렉터·같은 폭으로 재측정]
  I --> J{3px 넘는 줄?}
